...
class OrmGenerator {
	...
	/**
	 * @param Db $db
	 * @param string $autogeneratedPackage
	 ...
	 * @return void
	 */
	public function generate ($db, $autogeneratedPackage, $customPackage, $ignoreTables, $noInstantiateManagers, $positions) {
		$_gthis = $this;
		$autogenOrmClassName = $autogeneratedPackage . ".Orm";
		...
		foreach ($collection as $key => $value) {
			if (!in_array($value, $ignoreTables, false)) {
				$table = new GeneratorOrmTable($value, $autogeneratedPackage, $customPackage);
	...
	public function getAutogenOrm ($tables, $fullClassName, $noInstantiateManagers) {
		...
		$clas->addMethod("__construct", [new GeneratorPhpVar("db", GeneratorTools::toPhpType("nanodb.orm.Db"))], null, implode("\n", array_map(function ($t) {
			return "\$this->" . $t->varName . " = new " . GeneratorTools::toPhpType($t->customManagerClassName) . "(\$db, \$this);";
