...
class OrmGenerator {
	...
	public function __construct ($srcPath) {
		$this->srcPath = ($srcPath !== "" ? (rtrim(str_replace("\\", "/", $srcPath), "\\/")??'null') . "/" : "");
	}
	...
	public function generate ($db, $autogenPackage, $customPackage, $ignoreTables, $noInstantiateManagers, $positions) {
		...
		foreach ($collection as $key => $value) {
			if (!in_array($value, $ignoreTables, false)) {
				...
				(new GeneratorOrmQueryGenerator())->make($db, $table, $_gthis->srcPath, $positions);
	...
	public function makeAutogenOrm ($tables, $autogenOrmClassName, $customOrmClassName, $noInstantiateManagers) {
		...
		$destFileName = $this->srcPath . (str_replace(".", "/", $autogenOrmClassName)??'null') . ".php";
		GeneratorTools::mkdir(dirname($destFileName));
		file_put_contents($destFileName, "<?php\x0A// This file is autogenerated. Do not edit!\x0A\x0A" . ($autogenOrm->toString()??'null'));
	}
	...
	public function makeCustomOrm ($customOrmClassName, $autogenOrmClassName) {
		if (!file_exists($this->srcPath . "/" . (str_replace(".", "/", $customOrmClassName)??'null') . ".php")) {
			$customOrm = $this->getCustomOrm($customOrmClassName, $autogenOrmClassName);
			$destFileName = $this->srcPath . (str_replace(".", "/", $customOrmClassName)??'null') . ".php";
			GeneratorTools::mkdir(dirname($destFileName));
