...
class OrmGenerator {
	...
	public function generate ($db, $autogeneratedNamespace, $customNamespace, $ignoreTables, $noInstantiateManagers, $positions) {
		...
		$customEntityClassName = $customNamespace . ".Entity";
		$this1 = [];
		$tables = $this1;
		...
		foreach ($collection as $key => $value) {
			if (!in_array($value, $ignoreTables, false)) {
				...
				(new GeneratorOrmModelGenerator())->make($db, $table, $customOrmClassName, $_gthis->outPath, $positions, $customEntityClassName);
		...
		GeneratorLog::start("ENTITY => " . $customEntityClassName);
		$this->makeCustomEntity($customEntityClassName);
		GeneratorLog::finishSuccess();
	}
	 ...
	 * @return GeneratorPhpClass
	 */
	public function getCustomEntity ($fullClassName) {
		return new GeneratorPhpClass($fullClassName, "nanodb.orm.Entity");
	}
	 ...
	 * @return void
	 */
	public function makeCustomEntity ($customEntityClassName) {
		if (!file_exists($this->outPath . "/" . (str_replace(".", "/", $customEntityClassName)??'null') . ".php")) {
			$customOrm = $this->getCustomEntity($customEntityClassName);
