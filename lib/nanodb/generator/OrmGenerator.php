...
class OrmGenerator {
	...
	public function generate ($db, $autogeneratedNamespace, $customNamespace, $ignoreTables, $noInstantiateManagers, $positions) {
		...
		$customOrmEntityClassName = $customNamespace . ".OrmEntity";
		$this1 = [];
		$tables = $this1;
		...
		foreach ($collection as $key => $value) {
			if (!in_array($value, $ignoreTables, false)) {
				...
				(new GeneratorOrmModelGenerator())->make($db, $table, $customOrmClassName, $_gthis->outPath, $positions, $customOrmEntityClassName);
		...
		GeneratorLog::start("ENTITY => " . $customOrmEntityClassName);
		$this->makeAutogenOrmEntity($autogenOrmEntityClassName, $customOrmClassName);
		$this->makeCustomOrmEntity($customOrmEntityClassName, $autogenOrmEntityClassName);
		GeneratorLog::finishSuccess();
	}
	...
	public function getAutogenOrmEntity ($fullClassName, $customOrmClassName) {
		$klass = new GeneratorPhpClass($fullClassName, "nanodb.orm.Entity");
		$klass->addComment("// This file is autogenerated. Do not edit!");
	...
	/**
	 * @param string $fullClassName
	 * @param string $autogenOrmEntityClassName
	 * 
	 * @return GeneratorPhpClass
	 */
	public function getCustomOrmEntity ($fullClassName, $autogenOrmEntityClassName) {
		return new GeneratorPhpClass($fullClassName, $autogenOrmEntityClassName);
	}
	 ...
	 * @return void
	 */
	public function makeCustomOrmEntity ($customEntityClassName, $autogenEntityClassName) {
		if (!file_exists($this->outPath . "/" . (str_replace(".", "/", $customEntityClassName)??'null') . ".php")) {
			$customOrm = $this->getCustomOrmEntity($customEntityClassName, $autogenEntityClassName);
			$destFileName = $this->outPath . (str_replace(".", "/", $customEntityClassName)??'null') . ".php";
			GeneratorTools::mkdir(dirname($destFileName));
			file_put_contents($destFileName, $customOrm->toString());
		}
	}
