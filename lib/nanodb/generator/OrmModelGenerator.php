...
class OrmModelGenerator {
	...
	/**
	 * @param GeneratorPhpClass $klass
	 * @param GeneratorOrmPhpVar $v
	 * @param string $destCastPhpType
	 * 
	 * @return void
	 */
	...
	public function getAutogenModel ($table, $vars, $modelClassName, $customOrmClassName, $customEntityClassName) {
		...
		$klass->addMethod("__construct", [new GeneratorPhpVar("db", GeneratorTools::toPhpType("nanodb.orm.Db")), new GeneratorPhpVar("orm", GeneratorTools::toPhpType($customOrmClassName)), new GeneratorPhpVar("serializer", GeneratorTools::toPhpType("nanodb.orm.ISerializer"))], null, "\$this->db = \$db;\n\$this->orm = \$orm;\n\$this->serializer = \$serializer;");
		if (current(array_filter($vars, function ($v) {
			...
			$klass->addMethod("save", [], "void", "\$data = \$this->serializer->serializeObject(\$this, [ " . (implode(", ", array_map(function ($x) {
				return "'" . $x->name . "'";
			}, $savedVars))??'null') . " ]);\n" . "\$sets = []; foreach (\$data as \$k => \$v) \$sets[] = \"`\$k` = \" . \$this->db->quote(\$v);\n" . "\n" . "\$keys = \$this->serializer->serializeObject(\$this, [ " . (implode(", ", array_map(function ($x1) {
				return "'" . $x1->name . "'";
