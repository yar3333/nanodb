...
class OrmModelGenerator {
	...
	public function make ($db, $table, $customOrmClassName, $srcPath, $positions) {
		...
		$destFileName = $srcPath . (str_replace(".", "/", $table->autogenModelClassName)??'null') . ".php";
		...
		if (!file_exists($srcPath . "/" . (str_replace(".", "/", $table->customModelClassName)??'null') . ".php")) {
			$customModel = $this->getCustomModel($table->customModelClassName, $table->autogenModelClassName);
			$destFileName1 = $srcPath . (str_replace(".", "/", $table->customModelClassName)??'null') . ".php";
