...
class OrmManagerGenerator
{
	...
	public function createGetByMethodMany($vars, $modelClassName, $whereVars, $model, $positions)
    {
		...
		$method = 'getBy' . implode('And', array_map(static function ($v) { return Tools::fieldAsFunctionNamePart($v->haxeName); }, $whereVars));
    ...
    {
		if (!$whereVars) return;
        $method = 'getBy' . (implode('And', array_map(static function ($x) {
	...
	public function getAutogenManager($db, $table, $vars, $modelClassName, $autogeneratedManagerClassName, $customOrmClassName, $customManagerClassName, $queryClassName, $positions)
    {
		...
		$getVars = array_filter($vars, static function ($x) { return $x->isKey; });
		if ($getVars) {
		...
		$createVars = array_filter($vars, static function ($x1) { return !$x1->isAutoInc; });
		$autoIncVars = array_filter($vars, static function ($x2) { return $x2->isAutoInc; });
		$klass->addMethod('add', [new PhpVar('obj', Tools::toPhpType($modelClassName))], 'void', implode('', array_map(static function ($x3)  use (&$table, &$db, &$vars, &$_gthis) {
		...
		}, array_filter($createVars, static function ($x4)  use (&$positions) {
			return $positions->is($x4->table, $x4->name);
		}))) . '$data = $this->serializer->serializeObject($obj, [ ' . implode(', ', array_map(static function ($x5) {
			return "'" . $x5->name . "'";
		...
		if (current(array_filter($vars, static function ($v1) {
			return $v1->isKey;
		})) && current(array_filter($vars, static function ($v2) {
			return !$v2->isKey;
		}))) {
			$savedVars = array_filter($vars, static function ($v3) { return !$v3->isKey; });
			$whereVars = array_filter($vars, static function ($v4) { return $v4->isKey; });
			$klass->addMethod('save', [new PhpVar('obj', Tools::toPhpType($modelClassName)), new PhpVar('properties', 'string[]', 'null')], 'void', 'if ($properties === null) $properties = [ ' . implode(', ', array_map(static function ($x6) {
				return "'" . $x6->name . "'";
			}, $savedVars)) . " ];\n" . "\n" . "\$data = \$this->serializer->serializeObject(\$obj, \$properties);\n" . "\$sets = []; foreach (\$data as \$key => \$value) \$sets[] = \"`\$key` = \" . \$this->db->quote(\$value);\n" . "\n" . '$keys = $this->serializer->serializeObject($obj, [ ' . implode(', ', array_map(static function ($x7) {
				return "'" . $x7->name . "'";
		...
		$deleteVars = array_filter($vars, static function ($x8) { return $x8->isKey; });
		if (!$deleteVars)  $deleteVars = $vars;

		$klass->addMethod(
		    'deleteBy' . implode('And', array_map(static function ($x9) { return Tools::fieldAsFunctionNamePart($x9->haxeName); }, $deleteVars)),
            $deleteVars,
            'void',
		...
		foreach ($collection as $key => $value) {
			unset($fields);
			$fields = $value;
			$vs = array_filter($vars, static function ($x10)  use (&$fields) { return \in_array($x10->name, $fields, false); });
	...
	public function getForeignKeyVars($db, $table, $vars)
    {
        ...
        return array_filter($vars, static function ($v)  use (&$foreignKeys) {
            return current(array_filter($foreignKeys, static function ($fk)  use (&$v) {
    ...
    {
		$positionVar = array_filter($vars, static function($x) use($positions) {
			return $positions->is($x->table, $x->name);
		});
		if (\count($positionVar) === 0) return 'null';
        return "'" . implode(', ', array_map(static function ($x1) { return $x1->name; }, $positionVar)) . "'";
	}
	...
	public function getWhereSql($vars, $varPrefix = '')
    {
		...
		if (\count($vars) > 0) {
			return ' WHERE ' . implode(". ' AND ", array_map(static function($v)  use($varPrefix) { return '`' . $v->name . "` = ' . \$this->quote(\$" . $varPrefix . $v->haxeName . ')'; }, $vars));
		}
